...
class EffectsLibary {
  static Component sakeEffect(
    PositionComponent component,
  ) {
    component.add(SpriteComponent.fromImage(
  ...
  static Component bouncingScaleEffectOnce(PositionComponent component) {
    ...
    final BouncingScaleEffectOnce effect =
        BouncingScaleEffectOnce(componentEffectTo: component, duration: 1, scaleFrom: 0.8, isLoop: true, onComplete: () {});
    return effect;
  }

  static Component slotMachineWheelEffect(PositionComponent component) {
    final Vector2 sizeItem = Vector2(
      component.size.x / 2.5,
      component.size.x / 2.5,
    );
    final List<String> allId = ["zz", "tire", "angry", "money", "spiral", "circle", "smile", "x", "threeOval"];
    final SlotMachineReel _slotMachinReel = SlotMachineReel(
      allItemsId: allId,
      idItemsGet: ["angry", "spiral", "circle"],
      onStartWheel: () {},
      onCompleteWheel: () {},
      sizeItem: sizeItem,
      imageItemsWheel: Flame.images.fromCache(
        AppImages.column_slot_machine,
      ),
    );

    Future.delayed(Duration(milliseconds: 5000), () {
      _slotMachinReel.startWheel();
    });

    return AlignComponent(alignment: Anchor.center, child: _slotMachinReel);
  }

  static List<Component Function(PositionComponent component)> allEffects() =>
      [sakeEffect, boxBorderLightingStyle1, moneyChangeStyle1, floatingEffect, bouncingScaleEffectOnce, slotMachineWheelEffect];
}
